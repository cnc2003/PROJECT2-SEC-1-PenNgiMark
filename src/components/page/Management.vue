...
function confirmModalHandle(input) {
  ...
  if (input == "addMenu") {
    ...
    let editSpace = null
    let index = null
    function filterCateForEdit(category) {
      console.log(category)
      editSpace = filterResult.value.filter(
        (menuset) => menuset.category.toLowerCase() == category.toLowerCase()
      )
      return editSpace
    }
    function findEditIndex(menu_name) {
      index = editSpace.findIndex(
        (menu) => menu.menu_name.toLowerCase() == menu_name.toLowerCase()
      )
      return index
    }
    ...
    if (isEditMode.value) {
      ...
      if (editingItem.new_category) {
        ...
        const delobject = editSpace.menus.splice(index, 1)
        DeleteMenu(editSpace.id, delobject)
      } else {
        ...
        addNewMenu(editSpace.id, editSpace)
      }
    }
    ...
    if (!isEditMode) {
      if (editingItem.value.new_category) {
        ...
        addNewCategory(object)
      } else if (!editingItem.value.new_category) {
        filterCateForEdit(editingItem.value.category)
        findEditIndex(editingItem.value.menu_name)
